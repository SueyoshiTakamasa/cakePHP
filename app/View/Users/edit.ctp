<?php
	...
	$this->Html->addCrumb('メンバー編集', '');
?>
<div class="row">
    <?php echo $this->element('admins/navi'); ?>
	<div class="col-sm-10 pt-2 pb-5 px-4" >
        <h4 class="border-bottom pt-1 pb-2">メンバー編集</h4>
		<?php echo $this->Form->create('User'); ?>
			<table class="table table-bordered mt-4 w-75">
				<tr>
					<th class='align-middle bg-light fz-14' style="width:16%;">メンバーネーム<span class="badge badge-danger float-right">必須</span></th>
					<td class='px-4 py-3'>
		...
		<div class="">
			<?php echo $this->Form->submit('編集を確定する', array(
			   'div'    => false,
			   'escape' => false,
			   'class'  => 'btn btn-fb--green mt-2 pl-4 pr-4 h-25 fz-14',
			 ))?>
		</div>
		<?php
			echo $this->Form->end();
